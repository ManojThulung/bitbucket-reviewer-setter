...
(() => {
    ...
    const NO_OPTIONS_SELECTOR = '[class*="NoOptionsMessage"], [class*="noOptionsMessage"], [class*="no-options"]';
    ...
    window.addEventListener('sr-add-reviewer', (e) => {
        ...
        if ([...chips].some(c => chipHasName(c.textContent, reviewer.name))) {
                ...
                if (!options.length) {
                    ...
                    if (document.querySelector(NO_OPTIONS_SELECTOR)) {
                        if (!noOptionsSeenAt) {
                            ...
                            console.warn(`[SR] search returned no results for "${reviewer.name}"`);
                            finish({ ok: false, reason: `"${reviewer.name}" not found in search results` });
                        }
                    } else {
                        noOptionsSeenAt = 0;
                    }
        ...
        const observer = new MutationObserver(() => {
            ...
            queued = true;
            setTimeout(() => { queued = false; trySelect(); }, 16);
        });
